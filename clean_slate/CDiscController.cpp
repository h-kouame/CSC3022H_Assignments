...
#define LOG_FILE_1 "env_1_log.csv"
#define LOG_FILE_2 "env_2_log.csv"
#define LOG_FILE_3 "env_3_log.csv"
...
bool CDiscController::Update()
{
	...
	{		
		...
		if (m_iIterations < 1) {
			...
			os.open(file);
			os << "Mines Gathered" << ";" << "Dead Sweepers" << endl;
			os << sum << ";" << deaths << endl;
		}
		else {
			os.open(file, ios_base::app);
			os << sum << ";" << deaths << endl;
		}
		os.close();
